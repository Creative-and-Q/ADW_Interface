...
When finished, say: "IMPLEMENTATION COMPLETE"
     ...
     */
    async validateReactIcons(workingDir, code) {
        const invalidIcons = [];
        const suggestions = {};
        ...
        for (const match of importMatches) {
            ...
            try {
                ...
                console.warn(`CodingAgent: Could not validate react-icons for ${iconSet}:`, error.message);
            }
        }
        return {
     ...
     */
    async ensureCssModuleTypes(workingDir) {
...
`;
        ...
        try {
            ...
            console.warn(`CodingAgent: Could not create CSS module types:`, error.message);
        }
    }
    /**
     * Ensure tsconfig.json excludes test files to prevent test code from breaking builds
     */
    async ensureTsconfigExcludesTests(tsconfigPath) {
        try {
            ...
            console.warn(`CodingAgent: Could not update tsconfig at ${tsconfigPath}:`, error.message);
        }
    }
    /**
