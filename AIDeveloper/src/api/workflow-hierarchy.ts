 ...
 */
async function saveWorkflowArtifacts(
  workflowId: number,
  artifacts: Array<{ type: string; content: string; filePath?: string; metadata?: any }> | undefined
): Promise<void> {
  ...
  try {
    ...
    if (!workflow) {
      logger.error(
        ...
        new Error(`Workflow ${workflowId} not found`)
      );
      return;
    }

    if (workflow.type !== "feature") {
      logger.warn("executeFeatureWorkflow called with non-feature workflow", {
        workflowId,
        type: workflow.type,
      });
      return;
    ...
    const skipStatusUpdate =
      currentWorkflow &&
      ["pending_fix", "completed", "failed", "completed_with_warnings", "running"].includes(
    ...
    const {
      ...
      releaseWorkflowTreeLock: releaseLock,
      advanceSubWorkflowQueue: advanceNext,
    ...
    if (queueStatus !== "in_progress") {
      await updateSubWorkflowStatus(
        workflowId,
        queueStatus,
        queueStatus === "failed" ? result.summary : undefined
      );
    }
    await releaseLock(workflowId);

    logger.info("Feature workflow execution completed", {
      workflowId,
      status: finalStatus,
      queueStatus,
    });
    ...
    if (finalStatus === "completed" || finalStatus === "completed_with_warnings") {
      ...
      if (nextInQueue) {
        logger.info("Auto-advancing to next workflow in queue", {
          parentWorkflowId,
          nextWorkflowId: nextInQueue,
        });
      ...
      if (childNext) {
        logger.info("Auto-executing child sub-workflow", {
          parentWorkflowId: workflowId,
          nextWorkflowId: childNext,
        });
        const childWorkflow = await getWorkflow(childNext);
        if (
          childWorkflow &&
          ["feature", "bugfix", "refactor", "documentation", "review"].includes(childWorkflow.type)
        ) {
        ...
        if (updatedWorkflow?.status === "completed") {
          logger.info("All child sub-workflows completed - updating queue and advancing parent", {
            workflowId,
            parentWorkflowId,
          });
          ...
          if (siblingNext) {
            logger.info("Auto-advancing to next sibling workflow in queue", {
              parentWorkflowId,
              nextWorkflowId: siblingNext,
            });
            const siblingWorkflow = await getWorkflow(siblingNext);
            if (
              siblingWorkflow &&
              ["feature", "bugfix", "refactor", "documentation", "review"].includes(
                siblingWorkflow.type
              )
            ) {
              await executeFeatureWorkflow(
                siblingNext,
    ...
    const { updateSubWorkflowStatus, releaseWorkflowTreeLock: releaseLock } = await import(
    ...
    if (currentWorkflow?.status === WorkflowStatus.PENDING_FIX) {
      ...
      logger.info("Workflow has pending_fix status - bugfix sub-workflow is handling the error", {
        workflowId,
      });
      await releaseLock(workflowId);
      return;
...
async function executeChildrenRecursively(
  ...
  const { advanceSubWorkflowQueue, updateSubWorkflowStatus } = await import(
    "../sub-workflow-queue.js"
  );
  const { getWorkflow } = await import("../workflow-state.js");
  ...
  while (childNext) {
    logger.info("Executing child sub-workflow", {
      parentWorkflowId: workflowId,
      childWorkflowId: childNext,
    });
    const childWorkflow = await getWorkflow(childNext);
    if (
      childWorkflow &&
    ...
    if (nextWorkflowId) {
      (async () => {
        try {
          ...
          if (
            workflow &&
            ["feature", "bugfix", "refactor", "documentation", "review"].includes(workflow.type)
          ) {
            logger.info("Auto-executing sub-workflow after creation", {
            ...
            const orchestrator = new WorkflowOrchestrator();
            const payload =
              typeof workflow.payload === "string"
            ...
            if (!skipStatusUpdate) {
              // Update workflow status based on result
              subWorkflowFinalStatus = result.success
            ...
            let queueStatus: "completed" | "failed" | "in_progress" = "failed";
            if (
              subWorkflowFinalStatus === "completed" ||
              subWorkflowFinalStatus === "completed_with_warnings"
            ) {
              queueStatus = "completed";
            } else if (
              subWorkflowFinalStatus === "pending_fix" ||
              subWorkflowFinalStatus === "running"
            ) {
            ...
            logger.info("Sub-workflow execution completed", {
              workflowId: nextWorkflowId,
            ...
            if (
              ...
              if (subWorkflowFinalStatus === "running") {
                ...
                await executeChildrenRecursively(
                  nextWorkflowId,
                  getWorkflowOrchestratorPath,
                  getModuleDirectory
                );
                // After children complete, continue with siblings
              } else {
                logger.info(
                  "Sub-workflow did not complete successfully - stopping queue processing",
                  {
                    workflowId: nextWorkflowId,
                    status: subWorkflowFinalStatus,
                    message:
                      "Queue paused. Fix the issue or create a bugfix sub-workflow to retry.",
                  }
                );
            ...
            while (currentNext) {
              ...
              logger.info("Executing next sub-workflow in queue", {
              ...
              try {
                ...
                const loopSkipStatusUpdate =
                  ...
                  [
                    ...
                    "running",
                  ].includes(loopWorkflow.status);

                let nextFinalStatus: string;
                if (!loopSkipStatusUpdate) {
                  nextFinalStatus = nextResult.success
                    ? WorkflowStatus.COMPLETED
                    : WorkflowStatus.FAILED;
                  ...
                  logger.info(
                    "Skipping status update in loop - workflow already has final status",
                    {
                      workflowId: currentNext,
                ...
                if (
                  ...
                  nextFinalStatus === "completed_with_warnings"
                ) {
                ...
                if (loopQueueStatus !== "in_progress") {
                  await updateSubWorkflowStatus(
                    currentNext,
                    loopQueueStatus,
                    loopQueueStatus === "failed" ? nextResult.summary : undefined
                  );
                }
                ...
                logger.info("Sub-workflow in queue completed", {
                  workflowId: currentNext,
                  status: nextFinalStatus,
                  queueStatus: loopQueueStatus,
                });
                ...
                if (
                  ...
                  if (nextFinalStatus === "running") {
                    logger.info(
                      "Sub-workflow in loop has its own children - executing them recursively",
                      {
                        workflowId: currentNext,
                        status: nextFinalStatus,
                      }
                    ...
                    logger.info(
                      "Sub-workflow in loop did not complete successfully - stopping queue processing",
                      {
                        workflowId: currentNext,
                        status: nextFinalStatus,
                        message:
                ...
                logger.error("Failed to execute sub-workflow in queue", loopError as Error, {
                  workflowId: currentNext,
                });
                ...
                if (errorWorkflow?.status === WorkflowStatus.PENDING_FIX) {
                  logger.info(
                    "Workflow has pending_fix status in loop - bugfix sub-workflow is handling it",
                    { workflowId: currentNext }
                  );
                ...
                logger.warn("Sub-workflow in loop failed - queue paused", {
                  workflowId: currentNext,
                });
                break;
              }
          ...
          logger.error("Failed to auto-execute sub-workflow", error as Error, {
            workflowId: nextWorkflowId,
          });
          const { updateSubWorkflowStatus, releaseWorkflowTreeLock: releaseLock } = await import(
            "../sub-workflow-queue.js"
          ...
          if (errorWorkflow?.status === WorkflowStatus.PENDING_FIX) {
            logger.info("Workflow has pending_fix status - bugfix sub-workflow is handling it", {
              workflowId: nextWorkflowId,
            });
...
router.post("/:id/reset-and-retry", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await query(`DELETE FROM agent_executions WHERE workflow_id = ?`, [workflowId]);
    ...
    res.json({
      ...
      data: {
        ...
        statusSummary: {
          ...
          inProgress: allWorkflows.filter((w: any) =>
            ["planning", "coding", "testing", "running"].includes(w.status)
          ).length,
        },
      },
    });
