 ...
 * 7. Return success
 */
export async function switchBranchWithRebuild(targetBranch: string): Promise<BranchSwitchResult> {
  ...
  try {
    ...
    if (hasChanges) {
      return {
        success: false,
        message:
          "Cannot switch branches: You have uncommitted changes. Please commit or stash them first.",
